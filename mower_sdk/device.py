"""Einingklassar for handsaming av sky-MQTT."""
...
class NavimowCloudDevice:
    """Eining med tilkopling til sky-MQTT."""
    ...
    def __init__(self, cloud: NavimowCloud, device: Device, state_manager: StateManager) -> None:
        ...
        self.cloud.mqtt_event_message_event.add_subscribers(self._parse_message_for_device)
        self.cloud.mqtt_attributes_event.add_subscribers(self._parse_message_attributes_for_device)
        self.cloud.mqtt_state_event.add_subscribers(self._parse_message_state_for_device)

    def __del__(self) -> None:
        self.cloud.mqtt_event_message_event.remove_subscribers(self._parse_message_for_device)
        self.cloud.mqtt_attributes_event.remove_subscribers(
            self._parse_message_attributes_for_device
        )
        self.cloud.mqtt_state_event.remove_subscribers(self._parse_message_state_for_device)

    def set_notification_callback(
        self, func: Callable[[tuple[str, Optional[object]]], Awaitable[None]]
    ) -> None:
        self.state_manager.cloud_on_notification_callback.add_subscribers(func)

    async def _parse_message_attributes_for_device(self, event: DeviceAttributesMessage) -> None:
